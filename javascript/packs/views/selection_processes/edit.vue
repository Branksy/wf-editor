...
div(style="position:relative;width:600px")
  tag-form(ref='form' @submit='save' submit-button-text='Save' submit-button-icon='Save' submit-button-icon-accent='white' submit-button-accent='viridiangreen' submit-button-align='right')
    ...
    tag-form-group(v-if='form.available_talent_stages')
    ...
    tag-form-group(v-if='form.available_complete_stages')
...
export default {
  ...
  computed: {
    ...mapState({
      sp: 'selection_process',
      statusResponse: 'status'
    })
  },
  methods: {
    getData() {
      this.axios.get('/selection_processes/'+ this.$route.params.id +'/edit')
      .then(response => {
        if(response.status == 200) {
          Object.assign(this.$data, response.data.selection_process);
  ...
  watch: {
    statusResponse() {
      if(this.statusResponse == 200) this.$parent.$refs.editable.hide()
    },
    modal_count() {
      this.getData()
    }
