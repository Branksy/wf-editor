<template lang='pug'>
div(style="position:relative;width:600px;top: 100px" v-if='form.available_destination_states.length')
...
export default {
  ...
  computed: {
    ...mapState(['status', 'selection_process_tree']),
    action() {
  ...
  methods: {
    save() {
      this.$store.dispatch('update_transition', this.$data.tr)
      this.reenableForm()
    },
    refreshForm() {
      this.axios.post('/workflow_transitions/'+ this.item.id +'/refresh', { workflow_transition: this.tr })
      .then(response => {
  ...
  watch: {
    status() {
      if(this.status == 200) this.$parent.$refs.editable.hide();
    },
  ...
  created() {
    this.axios.get('/workflow_transitions/'+ this.item.id + '/edit')
    .then(response => {
      if(response.status = 200) {
        this.tr = response.data.transition
        this.tr.primary_communication_template = response.data.transition.primary_communication_template_id
