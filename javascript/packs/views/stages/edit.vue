...
div(style="position:relative;width: 600px" v-if='form.activity_types')
  tag-form(ref='form' @submit='save' submit-button-text='Save' submit-button-icon='Save' submit-button-icon-accent='white' submit-button-accent='viridiangreen' submit-button-align='right')
    ...
    tag-form-group(v-if="['offer'].include(activity_type)")
    ...
    tag-form-group
      tag-edit-field(name='days_in_stage' label='Days in stage until overdue' :value='days_in_stage_until_overdue' @valueChange='days_in_stage_until_overdue = $event.detail.value')
        tag-validation(rule='maxLength', args='100')
    tag-form-group
...
export default {
  ...
  data() {
    return {
      ...
      selection_process_id: this.$route.params.id
    }
  },
  computed: {
  ...
  methods: {
    save() {
      this.$store.dispatch('update_stage', this.$data)
      this.reenableForm()
    }
  },
  watch: {
  ...
  created() {
    this.axios.get('/workflow_stages/'+ this.item.id + '/edit')
    .then(response => {
